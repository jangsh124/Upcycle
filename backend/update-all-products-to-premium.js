...
mongoose.connect('mongodb://localhost:27017/auth-app')
  .then(async () => {
    ...
    try {
      // 1. deltajang@gmail.com 사용자 찾기
      const user = await User.findOne({email: 'deltajang@gmail.com'});
      if (!user) {
        console.log('User deltajang@gmail.com not found');
        return;
      }
      ...
      const result = await Product.updateMany(
        {'sellerId.email': 'deltajang@gmail.com'},
        {tier: 'premium'}
      );
      ...
      // 4. 업데이트된 상품들 확인
      const updatedProducts = await Product.find({'sellerId.email': 'deltajang@gmail.com'});
      console.log('\nUpdated products:');
